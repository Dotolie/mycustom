...
for i in root.iter('link'):
    ...
    if a.text is None:
        ...
        if ((temp.find(".txt")==-1) & (temp.find(".h")==-1) & (temp.find(".launch")==-1) & (temp.find(".sh")==-1) & (temp.find("README")==-1) & (temp.find(".cpp")==-1)):
...
for j in root.iter('configuration'):
    ...
    if temp == buildconfig:
        for i in j.iter('option'):
            ...
            if a == 'com.st.stm32cube.ide.mcu.gnu.managedbuild.tool.cpp.compiler.option.includepaths':
                for j in i.iter('listOptionValue'):
                    temp=j.get('value')
                    if temp != "":
                        temp=temp.replace("\\","/")
                        ...
                        temp=fullpath(temp)
                        #print(temp)
                        cppflags=cppflags+" -I"+temp+" \\\n"

            if a == 'com.st.stm32cube.ide.mcu.gnu.managedbuild.tool.cpp.compiler.option.definedsymbols':
                for j in i.iter('listOptionValue'):
                    temp=j.get('value')
                    if temp != "":
                        #print(temp)
                        cppflags=cppflags+" '-D"+temp+"' \\\n"




            if a == 'com.st.stm32cube.ide.mcu.gnu.managedbuild.tool.cpp.linker.option.script':
                temp=i.get('value')
                # New workaround to override value when configured with ${workspace_loc:/${ProjName}/xxx}
                temp = re.sub('\$\{[^:]*:/\$\{[^\}]*\}([^\}]*)\}', r'..\1', temp)
                temp=temp.replace("../", "",1)

...
if $(grep -q "stm32mp157[acdf]-sdds-mx" /proc/device-tree/compatible) ;
then
    board="STM32MP157A-SDDS"
    app="@default_fw@"
...
if [ -z "$(find @userfs_mount_point@/${app}/* -name ${app}.elf)" ]; then
    echo "The default copro example ${app} doesn't exist" > /dev/kmsg
    exit 1
else
    #copy ${board}_@default_fw@.elf into /lib/firmware/
    cp $(find @userfs_mount_point@/${app}/* -name ${app}.elf) /lib/firmware/.
fi
}

firmware_load_start() {
# Change the name of the firmware
echo -n ${app}.elf > /sys/class/remoteproc/remoteproc0/firmware
...
echo "Booting fw image ${app}.elf" > /dev/kmsg
}
...
if [ $(cat /sys/class/remoteproc/remoteproc0/state) == "running" ]; then
    ...
    echo "Stopping fw image ${app}.elf" > /dev/kmsg
else
